...
if mode == "Basic":
    ...
    st.subheader("🎭 Advanced Prediction with Actors")
    ...
    with st.sidebar:
        ...
        to_add = st.multiselect("Select Actors", actor_names)
        for actor in to_add:
        ...
        if selected_actors:
            ...
            for i, name in enumerate(selected_actors):
                ...
                with colA:
                    ...
                    with st.popover(f"{name} - Profile Details", use_container_width=True):
                            st.markdown(f"## {name}")
                    ...
                    if 'thumbnail' in response and 'source' in response['thumbnail']:
                        ...
                        image_response = get_cached_image(image_url)
                    ...
                    if pd.isna(row['Gender']) and 'description' in response:
                        actors_df.loc[actors_df['Name'] == name, 'Gender'] = response['description']
                    if pd.isna(row['Biography']) and 'extract' in response:
                        actors_df.loc[actors_df['Name'] == name, 'Biography'] = response['extract']
